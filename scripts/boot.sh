...
# Build the image
echo "Building image: stephenneal/laravel-helpers:${TAG}"
docker build -t stephenneal/laravel-helpers:"${TAG}" \
...
docker logs -f laravel-helpers

while true; do
    if [[ $(docker inspect -f '{{.State.Running}}' laravel-helpers) != true ]]; then
        break
    else
        echo "'laravel-helpers' container is still running... waiting 3 secs then checking again..."
        sleep 3
    fi
done

# Confirm it exited with code 0
if [[ $(docker inspect -f '{{.State.ExitCode}}' laravel-helpers) == 0 ]]; then
    echo "Success: Tests Passed! - stephenneal/laravel-helpers:${TAG}"
else
    echo "Error: Tests Failed! - stephenneal/laravel-helpers:${TAG}"
    exit 1
fi

# Confirm the image exists
docker image inspect stephenneal/laravel-helpers:"${TAG}" > /dev/null 2>&1
